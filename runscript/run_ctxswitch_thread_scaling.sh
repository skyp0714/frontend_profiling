...
OUT_CSV="${OUT_CSV:-${PROFILING_DIR}/results/contextswitch_thread_scaling.csv}"
# Keep the legacy thread-scaling figure name for compatibility.
OUT_PNG_THREAD="${OUT_PNG_THREAD:-${PROFILING_DIR}/results/contextswitch_percore_threads.png}"
OUT_PNG_FREQ="${OUT_PNG_FREQ:-${PROFILING_DIR}/results/contextswitch_frequency_vs_cores.png}"
PERF_CPU="${PERF_CPU:-0}"
...
bench_cmd() {
  ...
  case "${bench}" in
    tomcat)
      ...
      echo "JAVA_TOOL_OPTIONS=-XX:ActiveProcessorCount=${cores} SIZE=${TOMCAT_SIZE} DACAPO_CONVERGE=0 ITERATIONS=${TOMCAT_ITERATIONS} WATCHDOG_SECS=${TOMCAT_WATCHDOG_SECS} ./runscript/run_tomcat.sh"
      ;;
    finagle-http)
      echo "ACTIVE_CPUS=${cores} RUN_SECONDS=${FINAGLE_RUN_SECONDS} FINAGLE_PRINT_COMPILATION=1 ./runscript/run_finagle_http.sh"
      ;;
    finagle-chirper)
      echo "ACTIVE_CPUS=${cores} RUN_SECONDS=${FINAGLE_RUN_SECONDS} FINAGLE_PRINT_COMPILATION=1 ./runscript/run_finagle_chirper.sh"
      ;;
    *)
      return 1
      ;;
...
run_one() {
  ...
  raw_file="${PROFILING_DIR}/results/ctxswitch_threads_${bench}_${cores}c.perfraw"
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/plot_ctxswitch_thread_scaling.py" \
  --input "${OUT_CSV}" \
