...
OUT_CSV="${OUT_CSV:-${PROFILING_DIR}/results/kernel_idle_busy.csv}"
OUT_MD="${OUT_MD:-${PROFILING_DIR}/results/kernel_idle_busy.md}"
OUT_PERCORE_CSV="${OUT_PERCORE_CSV:-${PROFILING_DIR}/results/kernel_idle_busy_percore.csv}"
OUT_L1I_PNG="${OUT_L1I_PNG:-${PROFILING_DIR}/results/kernel_idle_busy_percore_l1i.png}"
OUT_L2I_PNG="${OUT_L2I_PNG:-${PROFILING_DIR}/results/kernel_idle_busy_percore_l2i.png}"
...
idle_raw="${PROFILING_DIR}/results/kernel_idle_percore.perfraw"
busy_raw="${PROFILING_DIR}/results/kernel_busy_percore.perfraw"
...
setsid taskset -c "${CORES}" bash -lc \
  "cd '${PROFILING_DIR}' && JAVA_TOOL_OPTIONS=-XX:ActiveProcessorCount=${TOMCAT_CORES} SIZE=${TOMCAT_SIZE} DACAPO_CONVERGE=0 ITERATIONS=${TOMCAT_ITERATIONS} WATCHDOG_SECS=${TOMCAT_WATCHDOG_SECS} ./runscript/run_tomcat.sh" \
  > "${busy_log}" 2>&1 &
busy_pid=$!
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/build_kernel_idle_busy_csvs.py" \
  --idle-raw "${idle_raw}" \
  --busy-raw "${busy_raw}" \
  --cores "${CORES}" \
  --idle-secs "${IDLE_SECS}" \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/plot_kernel_idle_busy_percore.py" \
