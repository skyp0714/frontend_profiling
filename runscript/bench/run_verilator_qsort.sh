...
if [[ "${FORCE_REBUILD:-0}" == "1" ]]; then
  ...
  echo "[err] Run ./runscript/build/build_verilator_qsort.sh separately before profiling." >&2
  exit 1
fi
...
if [[ -z "${EMU}" ]]; then
  ...
  echo "[err] build first: CHIPYARD_CONFIG=${CHIPYARD_CONFIG} ./runscript/build/build_verilator_qsort.sh" >&2
  exit 1
fi
