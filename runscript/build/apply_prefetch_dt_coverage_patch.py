...
def parse_args():
    ap = argparse.ArgumentParser(
        description=(
            "Apply dt coverage prefetch insertions to generated Verilator C++. "
            "Resolved targets use source labels; line-0/unresolved targets use direct symbol+offset operands."
        )
    )
...
def precompute_body_starts(lines: list[str]):
    # Verilator's largest generated functions can confuse a naive brace
    # counter because the files are enormous and contain macro-heavy source.
    # For patching known addr2line locations, the nearest preceding function
    # signature is a more robust and much faster anchor.
    body_starts: list[int | None] = [None] * len(lines)
    body_start = None
    for i, line in enumerate(lines):
        if FUNC_DEF_RE.match(line):
            body_start = i + 1
        body_starts[i] = body_start
    return body_starts


def find_statement_insert_index_fast(lines: list[str], body_starts: list[int | None], line_no: int):
    idx = max(0, min(line_no - 1, len(lines)))
    if not lines:
        return None
    bounded_idx = min(idx, len(lines) - 1)
    body_start = body_starts[bounded_idx] if 0 <= bounded_idx < len(body_starts) else None
    if body_start is None:
        # Signature-line and confused-brace fallbacks are rare; keep the slow
        # but battle-tested path for those instead of complicating the fast map.
        return find_statement_insert_index(lines, line_no)
    idx = max(idx, body_start)
    for j in range(idx - 1, body_start - 1, -1):
        stripped = lines[j].strip()
        if not stripped or stripped.startswith("//"):
...
def load_variant_points(result_root: Path, variant_short: str):
    ...
    with csv_path.open("r", encoding="utf-8", newline="") as f:
        for row in csv.DictReader(f):
            ...
            operand = row.get("target_operand", "").strip()
            if not rel or not line_s or (not label and not operand):
                continue
            try:
                line_no = int(line_s)
            except ValueError:
...
def load_variant_labels(result_root: Path, variant_short: str):
    ...
    with csv_path.open("r", encoding="utf-8", newline="") as f:
        for row in csv.DictReader(f):
            ...
            label = row.get("target_label", "").strip()
            kind = row.get("target_kind", "").strip()
            operand_type = row.get("target_operand_type", "").strip()
            if kind == "offset" or operand_type == "symbol_offset":
                continue
...
def make_prefetch_block(rows: list[dict], variant_short: str, rel: str, requested_line: int, safe_line: int):
    ...
    for row in rows:
        ...
        operand = row.get("target_operand", "").strip() or label
        ...
        for mnemonic in prefetch_mnemonics(kind):
            for i in range(line_count):
                off = i * line_size
                label_expr = operand if off == 0 else f"{operand}+{off}"
                block.append(
                    (
                        ...
                        f"target_label={label} target_operand={operand} target_kind={row.get('target_kind','')} "
                        f"site_class={row.get('site_class','')} lead_bytes={row.get('lead_bytes','')} "
...
def patch_variant(variant_dir: Path, config: str, variant_short: str, result_root: Path):
    ...
    for rel in all_rels:
        ...
        body_starts = precompute_body_starts(lines)
        ...
        for line_no, rows in prefetch_grouped.get(rel, {}).items():
            if line_no < 1 or line_no > len(lines) + 1:
                ...
                continue
            insert_idx = find_statement_insert_index_fast(lines, body_starts, line_no)
...
def main():
    ...
    expected_labels = 0
    labels_csv = Path(args.result_root).resolve() / "target_labels.csv"
    if labels_csv.is_file():
        with labels_csv.open("r", encoding="utf-8", newline="") as f:
            for row in csv.DictReader(f):
                if row.get("variant_short", "").strip() == args.variant_short:
                    expected_labels += 1
    if args.variant_short != "baseline" and expected_labels > 0 and inserted_labels <= 0:
